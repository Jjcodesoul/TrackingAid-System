...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="url('/requests')" :active="request()->is('requests*')">
                        {{ __('Requests') }}
                    </x-nav-link>
                    ...
                    @if(Auth::check() && Auth::user()->role === 'admin')
                        <x-nav-link :href="url('/stock-in')" :active="request()->is('stock-in*')">
                            {{ __('Stock In') }}
                        </x-nav-link>

                        <x-nav-link :href="url('/borrow-release')" :active="request()->is('borrow-release*')">
                            {{ __('Borrow / Release') }}
                        </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="url('/requests')" :active="request()->is('requests*')">
                {{ __('Requests') }}
            </x-responsive-nav-link>

            <x-responsive-nav-link :href="url('/inventory')" :active="request()->is('inventory*')">
                {{ __('Inventory') }}
            </x-responsive-nav-link>

            @if(Auth::check() && Auth::user()->role === 'admin')
                <x-responsive-nav-link :href="url('/stock-in')" :active="request()->is('stock-in*')">
                    {{ __('Stock In') }}
                </x-responsive-nav-link>

                <x-responsive-nav-link :href="url('/borrow-release')" :active="request()->is('borrow-release*')">
                    {{ __('Borrow / Release') }}
                </x-responsive-nav-link>
